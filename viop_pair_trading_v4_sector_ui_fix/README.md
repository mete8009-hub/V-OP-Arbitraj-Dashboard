...
## v3 — Pair Trading Screener

Bu sürümde mevcut Spot ↔ VİOP baz dashboard'una üçüncü bir mod eklendi:
...
- BIST hisseleri için yfinance üzerinden tarihsel düzeltilmiş kapanış fiyatı çeker.
- Seçilen evrendeki tüm ikili hisse kombinasyonlarını test eder.
- Log fiyatlar üzerinden hedge ratio hesaplar.
- Engle-Granger koentegrasyon testi uygular.
- Spread için ADF testi, rolling z-score ve half-life hesaplar.
- Z-score mean-reversion mantığıyla basit backtest üretir.
- Pair'leri skor, p-value, z-score, Sharpe, drawdown, işlem sayısı ve VİOP uygulanabilirliği ile sıralar.

### Yeni dosyalar

```text
src/historical_fetcher.py   # yfinance tarihsel veri çekme katmanı
src/pair_trading.py         # koentegrasyon, z-score, backtest ve skor motoru
```
...
İlk denemede tüm BIST yerine 15-50 arası likit hisse seçmek daha sağlıklıdır. 50 hisse yaklaşık 1.225 pair, 100 hisse yaklaşık 4.950 pair üretir. Çok geniş evren Streamlit Cloud'da yavaşlayabilir.

### Önemli sınırlama

VİOP kontratları kısa vadeli olduğu için 1/3/5/10 yıllık koentegrasyon testi spot hisseler üzerinden yapılır. Bulunan pair için iki bacakta da güncel VİOP kontratı varsa dashboard bunu `viop_available=True` olarak işaretler. Gerçek işlem öncesi short erişimi, ödünç maliyeti, VİOP teminatı, bid-ask ve slippage ayrıca kontrol edilmelidir.


---

## v4 sektör filtresi ve UI düzeltmeleri

- Pair Trading Screener ekranına sektör filtresi eklendi.
- `Sadece aynı sektör pair'leri` seçeneği eklendi.
- Sonuç tablosuna `sector_a`, `sector_b`, `same_sector` kolonları eklenebilir hale getirildi.
- Plotly grafiklerinde başlık, eksen, hover, çizgi ve grid renkleri okunabilir olacak şekilde sabitlendi.
- Sidebar, buton, multiselect tag ve uyarı kutuları için okunabilir açık tema CSS'i güçlendirildi.
